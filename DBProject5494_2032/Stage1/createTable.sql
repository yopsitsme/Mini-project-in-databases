...
-- Create trigger function to update current_amount when student joins group
CREATE OR REPLACE FUNCTION update_current_amount_on_insert()
RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger function to update current_amount when student leaves group
CREATE OR REPLACE FUNCTION update_current_amount_on_delete()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE group_of_sports 
    SET current_amount = current_amount - 1 
...
-- Attach triggers to participate_in table
